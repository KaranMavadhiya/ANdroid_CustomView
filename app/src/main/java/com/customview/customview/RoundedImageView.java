...
public class RoundedImageView extends AppCompatImageView {
    ...
    private void updateAttrs(Drawable drawable, boolean background) {
        ...
        if (drawable instanceof RoundedDrawable) {
            ((RoundedDrawable) drawable).setScaleType(mScaleType).setCornerRadius(background && !mRoundBackground ? 0 : mCornerRadius)
                    .setBorderWidth(background && !mRoundBackground ? 0 : mBorderWidth).setBorderColors(mBorderColor).setOval(mOval);
